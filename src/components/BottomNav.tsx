  ...
  return (
    <div className="md:hidden fixed bottom-0 w-full bg-secondary border-t border-primary/10 shadow-[0_-2px_10px_rgba(0,0,0,0.05)] z-50 px-2 py-2 safe-area-bottom">
      <div className="flex justify-around items-center">
        <NavItem to="/" icon={<Home size={22} />} label="ہوم" />
        <NavItem to="/search" icon={<Search size={22} />} label="تلاش" />
        <NavItem to="/categories" icon={<BookOpen size={22} />} label="زمرہ جات" />
        <NavItem to="/ask" icon={<MessageCircleQuestion size={22} />} label="سوال پوچھیں" />
      </div>
    </div>
  );
}
